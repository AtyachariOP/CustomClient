...
import { useVirtualizer } from '@tanstack/react-virtual';
  import { Search, ChevronDown, ChevronUp, Download, ArrowLeft, CheckCircle, ExternalLink, Loader2, AlertCircle } from 'lucide-react';
...
export default function Marketplace() {
  ...
  useEffect(() => {
    ...
    fetch('https://api.modrinth.com/v2/tag/game_version')
      .then(res => res.json())
      .then(data => {
        setGameVersions(data.filter((v: any) => v.version_type === 'release').map((v: any) => v.version));
      })
      .catch(console.error);
  ...
  const fetchMods = async () => {
    ...
    try {
      ...
      if (selectedVersions.length > 0) {
        facetsList.push(`[` + selectedVersions.map(v => `"versions:${v}"`).join(',') + `]`);
      }
      ...
      setApiError("Failed to connect to the Modrinth API. Please check your internet connection or try again later.");
    }
  ...
  }, [query, selectedCategories, selectedEnvironments, selectedVersions, openSourceOnly, projectType, sort, offset]);
          ...
          <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '16px', marginTop: '24px', alignItems: 'center' }}>
                ...
                <a href={`https://modrinth.com/mod/${viewingMod.project_id}`} target="_blank" rel="noreferrer" className="glass btn" style={{ padding: '12px 24px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none', color: 'var(--text-primary)' }}>
  ...
  return (
    <div className="animate-fade" style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', height: '100%', gap: '24px', overflow: 'hidden' }}>
        {/* Main Mods Area */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px', minWidth: '500px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                ...
                <div 
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-primary)', padding: '6px 12px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', minWidth: '130px', justifyContent: 'space-between', transition: 'var(--transition-fast)' }}
                  ...
                  <div className="glass animate-fade" style={{ position: 'absolute', top: '100%', left: '50px', marginTop: '8px', width: '160px', borderRadius: '12px', padding: '6px', display: 'flex', flexDirection: 'column', gap: '2px', zIndex: 100, background: 'rgba(20, 21, 26, 0.95)', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }}>
                    {[
                      ...
                      <button 
                        ...
                        style={{ background: sort === opt.value ? 'var(--accent-primary)' : 'transparent', color: 'var(--text-primary)', border: 'none', padding: '8px 12px', borderRadius: '6px', textAlign: 'left', fontSize: '13px', cursor: 'pointer' }}
                        onMouseOver={(e) => { 
          ...
          <div 
            ...
            style={{ flex: 1, overflowY: 'auto', paddingRight: '8px', position: 'relative' }}
            onScroll={(e) => {
          ...
          >
            {apiError && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60%', color: 'var(--text-secondary)', marginTop: '48px' }}>
                <AlertCircle size={64} style={{ marginBottom: '16px', color: '#ef4444', opacity: 0.8 }} />
                <h3 style={{ fontSize: '20px', fontWeight: 600, color: 'white', marginBottom: '8px' }}>API Error</h3>
                <p style={{ maxWidth: '400px', textAlign: 'center', lineHeight: '1.5' }}>{apiError}</p>
                <button className="btn btn-primary btn-lift" onClick={() => fetchMods()} style={{ marginTop: '24px', padding: '10px 24px', borderRadius: '8px' }}>
                  Retry Connection
                </button>
              </div>
            )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <button 
                        ...
                        onClick={(e) => { e.stopPropagation(); if (!isInstalled) setInstallingMod(mod); }}
      ...
      <div className="glass" style={{ width: '260px', display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto', flexShrink: 0, padding: '20px', borderRadius: '16px', background: 'rgba(15, 15, 18, 0.45)' }}>
        ...
        {/* Game Version */}
        <div>
          <h4 onClick={() => toggleSection('gameVersion')} style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: expandedSections.gameVersion ? '12px' : '0', display: 'flex', justifyContent: 'space-between', paddingBottom: '8px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}>
            Game version {expandedSections.gameVersion ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
          </h4>
          {expandedSections.gameVersion && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(0,0,0,0.2)', padding: '6px 8px', borderRadius: '6px', marginBottom: '4px' }}>
                ...
                .map(v => {
                  ...
                  return (
                    <button 
                      ...
                      onClick={() => {
                        setMods([]); setOffset(0);
                        if(isActive) setSelectedVersions(selectedVersions.filter(x => x !== v));
                        else setSelectedVersions([...selectedVersions, v]);
                      }}
                      style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px', color: isActive ? 'white' : 'var(--text-secondary)', background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px 0', textAlign: 'left' }}
                    >
                      <div style={{ width: '16px', height: '16px', border: isActive ? 'none' : '1px solid rgba(255,255,255,0.2)', borderRadius: '4px', background: isActive ? 'var(--accent-primary)' : 'rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              {gameVersions.filter(v => v.includes(versionSearch)).length > 6 && (
                <button onClick={() => setShowAllVersions(!showAllVersions)} style={{ background: 'transparent', border: 'none', color: 'white', fontSize: '12px', marginTop: '4px', cursor: 'pointer', textAlign: 'left', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ width: '16px', height: '16px', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '4px', background: 'rgba(0,0,0,0.2)' }} />
                  {showAllVersions ? 'Show less versions' : 'Show all versions'}
                </button>
              )}
            </div>
          )}
        ...
        {/* Loader */}
        <div>
          <h4 onClick={() => toggleSection('loader')} style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: expandedSections.loader ? '12px' : '0', display: 'flex', justifyContent: 'space-between', paddingBottom: '8px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}>
            Loader {expandedSections.loader ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
          </h4>
          {expandedSections.loader && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {['Fabric', 'Forge', 'NeoForge'].map(loader => {
                ...
                return (
                  <div key={loader} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px', color: isFabric ? 'white' : 'var(--text-secondary)', padding: '4px 0', opacity: isFabric ? 1 : 0.5 }}>
                    <div style={{ width: '16px', height: '16px', border: isFabric ? 'none' : '1px solid rgba(255,255,255,0.2)', borderRadius: '4px', background: isFabric ? '#f29c38' : 'rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ color: 'var(--text-secondary)', fontSize: '12px', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
        ...
        {/* Category */}
        <div>
          <h4 onClick={() => toggleSection('category')} style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: expandedSections.category ? '12px' : '0', display: 'flex', justifyContent: 'space-between', paddingBottom: '8px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}>
            Category {expandedSections.category ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
          </h4>
          {expandedSections.category && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
               {categoriesList.slice(0, showMoreCategories ? undefined : 9).map(cat => {
               ...
               {categoriesList.length > 9 && (
                 <button onClick={() => setShowMoreCategories(!showMoreCategories)} style={{ background: 'transparent', border: 'none', color: 'var(--accent-primary)', fontSize: '12px', marginTop: '8px', cursor: 'pointer', textAlign: 'left', fontWeight: 600 }}>
                   {showMoreCategories ? '- View Less' : `+ View ${categoriesList.length - 9} More`}
                 </button>
               )}
            </div>
          )}
        ...
        {/* Environment */}
        <div>
          <h4 onClick={() => toggleSection('environment')} style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: expandedSections.environment ? '12px' : '0', display: 'flex', justifyContent: 'space-between', paddingBottom: '8px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}>
            Environment {expandedSections.environment ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
          </h4>
        ...
        {/* License */}
        <div>
          <h4 onClick={() => toggleSection('license')} style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: expandedSections.license ? '12px' : '0', display: 'flex', justifyContent: 'space-between', paddingBottom: '8px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}>
            License {expandedSections.license ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
          </h4>
          {expandedSections.license && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
               <button 
                 onClick={() => { setMods([]); setOffset(0); setOpenSourceOnly(!openSourceOnly); }}
                 style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px', color: openSourceOnly ? 'white' : 'var(--text-secondary)', background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px 0', textAlign: 'left' }}
               >
                 <div style={{ width: '16px', height: '16px', border: openSourceOnly ? 'none' : '1px solid rgba(255,255,255,0.2)', borderRadius: '4px', background: openSourceOnly ? 'var(--accent-primary)' : 'rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
