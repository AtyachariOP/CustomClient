...
import { Search, Plus, Folder, Settings, ChevronDown, Box, CheckCircle, PackageOpen } from 'lucide-react';
import ModpackWindow from './ModpackWindow';
import { useLocalStorage } from '../hooks/useLocalStorage';
import { defaultProfiles } from '../utils/defaultProfiles';

export default function Library() {
  const [profiles] = useLocalStorage<any[]>('launcher_profiles', defaultProfiles);
  const [selected, setSelected] = useState(profiles.length > 0 ? profiles[0] : null);
  ...
  return (
    <div className="animate-fade" style={{ display: 'flex', height: '100%', gap: '24px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto', paddingRight: '8px', minWidth: '500px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div className="glass" style={{ flex: 1, display: 'flex', alignItems: 'center', padding: '0 16px', borderRadius: '12px', height: '42px' }}>
            ...
            <input type="text" placeholder="Search your modpacks..." style={{ background: 'transparent', border: 'none', color: 'var(--text-primary)', outline: 'none', width: '100%', fontSize: '14px' }} />
          </div>
          <button className="glass btn" style={{ padding: '10px 20px', borderRadius: '12px' }}>
        ...
        {profiles.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', opacity: 0.5 }}>
            ...
            <h3 style={{ fontSize: '18px', fontWeight: 600, color: 'white', marginBottom: '8px' }}>No Profiles Found</h3>
            ...
            {profiles.map(pack => (
              ...
              >
                ...
                <div style={{ position: 'absolute', inset: 0 }}>
                  <img src={pack.bg} alt={pack.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ...
                <div style={{ position: 'absolute', inset: 0, padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    <img src={pack.icon} style={{ width: '28px', height: '28px', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }} />
                    <h3 style={{ fontSize: '18px', fontWeight: 800, textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>{pack.title}</h3>
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 600, background: 'rgba(255,255,255,0.2)', padding: '2px 8px', borderRadius: '4px', backdropFilter: 'blur(4px)' }}>{pack.version}</span>
                    <span style={{ fontSize: '12px', fontWeight: 600, background: 'rgba(0,0,0,0.5)', padding: '2px 8px', borderRadius: '4px' }}>By {pack.author}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="glass" style={{ width: '340px', display: 'flex', flexDirection: 'column', borderRadius: '24px', padding: '24px', overflowY: 'auto', background: 'linear-gradient(180deg, rgba(20,21,26,0.8), rgba(15,15,18,0.95))', border: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '32px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <div style={{ position: 'relative' }}>
                   <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: '#1a1b1e' }} />
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
              <button className="glass" onClick={() => setIsModpackWindowOpen(true)} style={{ padding: '16px', borderRadius: '16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', transition: 'var(--transition-fast)' }} onMouseOver={e => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'} onMouseOut={e => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}>
      ...
      {isModpackWindowOpen && selected && (
        <ModpackWindow modpack={selected} close={() => setIsModpackWindowOpen(false)} />
      )}
    </div>
  );
}
